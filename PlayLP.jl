...
function showpolicy(profile, hand, hist)
    hand = map(n -> UInt8(n), hand)
    hist = map(n -> UInt8(n), hist)
    k = key(hand, hist)
    policy = profile.policys[k].strategysum |> normalize
    for (k, v) in policy
        if v >= 0.01
            @printf("%2.0f => %0.2f\n", k, v)
        end
...
function randplay(profile, dict1, dict2, n)
    total = 0
    hs = Matrix{UInt8}(undef, (2, quant))
    for _ = 1:n
        (_, x, _) = headsup(dict1, dict2, randhands!(profile, hs))
        total += x
    end
...
function play(profile)
    hist = UInt8[]
    hands = Matrix{UInt8}(undef, (2, profile.cards_per_hand))
    randhands!(profile, hands)
    you = rand(1:2)
    her = 3 - you
    player = 1
    ...
    while true
        if player == you
            ...
            t = readline()
            mv = tryparse(UInt8, t)
            if mv ∉ actions(profile.all_bids, hist)
                println("XXX - Illegal action, please try again - XXX")
                continue
            end
            player = her
        else
            mv = move(profile.policys, hands[player, :], hist)
            player = you
            if mv == 0
...
function games(profile)
    ...
    score = 0
    println("----------------------------")
    @printf("Liars Poker: %.0f Card, %.0f Ranks\n", profile.cards_per_hand, profile.num_ranks)
